...
static void draw_gauge(lv_obj_t *canvas, const struct status_state *state, int y_offset) {
    lv_draw_img_dsc_t img_dsc;
    lv_draw_img_dsc_init(&img_dsc);
    // Restore original position (y=70)
    lv_canvas_draw_img(canvas, 0, 70 + y_offset, &gauge, &img_dsc);
}

static void draw_needle(lv_obj_t *canvas, const struct status_state *state, int y_offset) {
    lv_draw_line_dsc_t line_dsc;
    init_line_dsc(&line_dsc, LVGL_FOREGROUND, 1);
    int centerX = 12; // 16 default
    int centerY = 90 + y_offset; // 100 gut, 66 default
...
static void draw_grid(lv_obj_t *canvas, int y_offset) {
    lv_draw_img_dsc_t img_dsc;
    lv_draw_img_dsc_init(&img_dsc);
    // Restore original position (y=95)
    lv_canvas_draw_img(canvas, -1, 95 + y_offset, &grid, &img_dsc);
}

static void draw_graph(lv_obj_t *canvas, const struct status_state *state, int y_offset) {
    lv_draw_line_dsc_t line_dsc;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        points[i].x = -36 + i * 7.4;
        points[i].y = 127 - (value * 32 / max) + y_offset;
    }
#else
    int max = 0;
    int min = 256;
    ...
    for (int i = 0; i < 10; i++) {
        points[i].x = 0 + i * 7.4;
        points[i].y = 97 - (state->wpm[i] - min) * 32 / range + y_offset;
    }
#endif
...
static void draw_label(lv_obj_t *canvas, const struct status_state *state, int y_offset) {
    ...
    if (state->wpm[9] < 10) {
        lv_canvas_draw_text(canvas, 12, 75 + y_offset, 50, &label_dsc_wpm, wpm_text);
    } else if (state->wpm[9] >= 10 && state->wpm[9] < 100) {
        lv_canvas_draw_text(canvas, 9, 75 + y_offset, 50, &label_dsc_wpm, wpm_text);
    } else {
        lv_canvas_draw_text(canvas, 7, 75 + y_offset, 50, &label_dsc_wpm, wpm_text);
    }
}

void draw_wpm_status(lv_obj_t *canvas, const struct status_state *state, int y_offset) {
    draw_gauge(canvas, state, y_offset);
    draw_needle(canvas, state, y_offset);
    draw_grid(canvas, y_offset);
    draw_graph(canvas, state, y_offset);
    draw_label(canvas, state, y_offset);
}
